import { A as AstroError, y as ActionNotFoundError } from './astro/server_DHY1K0at.mjs';

async function getAction(path) {
  const pathKeys = path.split(".").map((key) => decodeURIComponent(key));
  let { server: actionLookup } = await import('./_astro_internal-actions_rQXTe_XK.mjs');
